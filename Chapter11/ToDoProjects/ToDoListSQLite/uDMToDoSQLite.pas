...
type
  TDMToDoSQLite = class(TDataModule, IToDoData)
...
function TDMToDoSQLite.IsMobilePlatform: boolean;
begin
  Result := (TOSVersion.Platform = pfiOS) or (TOSVersion.Platform = pfAndroid);
end;

procedure TDMToDoSQLite.fdconnToDosBeforeConnect(Sender: TObject);
begin
...
procedure TDMToDoSQLite.fdconnToDosAfterConnect(Sender: TObject);
const
...
function TDMToDoSQLite.GetNewId: integer;
var fld: TField;
begin
  fdqToDoMaxId.Open;
  try
...
function TDMToDoSQLite.ToDoCreate(aValue: TToDo): integer;
var id: integer;
begin
  id := GetNewId;
...
function TDMToDoSQLite.ToDoRead(id: integer; out aValue: TToDo): boolean;
begin
...
function TDMToDoSQLite.ToDoUpdate(aValue: TToDo): boolean;
begin
...
function TDMToDoSQLite.ToDoDelete(id: integer): boolean;
begin
...
procedure TDMToDoSQLite.ToDoList(aList: TToDos);
var item: TToDo;
begin
  if aList <> nil then
  begin

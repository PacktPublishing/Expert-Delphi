...
var
  FormToDoSQLite: TFormToDoSQLite;
...
procedure TFormToDoSQLite.FormCreate(Sender: TObject);
begin
...
procedure TFormToDoSQLite.FormDestroy(Sender: TObject);
begin
  FToDos.Free;
end;

function TFormToDoSQLite.GetToDoData: IToDoData;
begin
  if DMToDoSQLite = nil then
    DMToDoSQLite := TDMToDoSQLite.Create(Application);
  Result := DMToDoSQLite;
end;

procedure TFormToDoSQLite.actAddExecute(Sender: TObject);
begin
  FCurrentId := -1;
  edtTitle.Text := '';
  edtCategory.Text := '';
...
procedure TFormToDoSQLite.actDeleteExecute(Sender: TObject);
begin
...
procedure TFormToDoSQLite.actSaveExecute(Sender: TObject);
var todo: TToDo;
begin
...
procedure TFormToDoSQLite.lstvwToDosDeleteItem(Sender: TObject; AIndex: Integer);
begin
  FCurrentId := FToDos[AIndex].Id;
  actDelete.Execute;
end;

procedure TFormToDoSQLite.lstvwToDosItemClick(const Sender: TObject;
  const AItem: TListViewItem);
var todo: TToDo;
begin
...
procedure TFormToDoSQLite.RefreshList;
